...
# a custom dataset for imagenette that inherits from the pytorch dataset class
class IMAGENETTE(data.Dataset):
    def __init__(self, root, train = True, transform=None, target_transform=None, info_transform=None, download=False):
        self.root = os.path.expanduser(os.path.join(root, 'imagenette2'))
        self.transform = transform
        self.target_transform = target_transform
        self.info_transform = info_transform
        ...
        self.data, self.targets, self.embeds, self.sample_indices = self._load_samples()

    def _load_samples(self):
        """Load image samples from dataset folder."""
        data = []
        targets = []
        embeds = []
        sample_indices = []
        images_path = os.path.join(self.root, self.split)
        embeds_path = os.path.join(self.root, f'{self.split}_masks_embeds')
        for class_idx, class_name in enumerate(sorted(os.listdir(images_path))):
            class_imgs_dir = os.path.join(images_path, class_name)
            class_embeds_dir = os.path.join(embeds_path, class_name)
            for sample_idx, sample_name in enumerate(sorted(os.listdir(class_imgs_dir))):
                sample_path = os.path.join(class_imgs_dir, sample_name)
                embed_path = os.path.join(class_embeds_dir, sample_name).replace('.JPEG', '.pkl')
                if os.path.isfile(sample_path):
                    data.append(sample_path)
                    targets.append(class_idx)
                    embeds.append(embed_path)
                    sample_indices.append(sample_idx)
        return data, targets, embeds, sample_indices
    
    def _check_exists(self):
        return os.path.exists(os.path.join(self.root, self.split))
    
    def __getitem__(self, index):
        """Get image sample by index."""
        data, target, embed_path, sample_idx = self.data[index], self.targets[index], self.embeds[index], self.sample_indices[index]
        ...
        if self.target_transform is not None:
            target = self.target_transform(torch.tensor(target))
        
        return image, target, embeds, sample_idx
    def __len__(self):
        """Return dataset size."""
        return len(self.data)
    
    def extra_repr(self):
        """Set the extra representation of the dataset."""
...
class IMAGENETTEDataModule(VisionDataModule):    
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            if not _TORCHVISION_AVAILABLE:  # pragma: no cover
                raise ModuleNotFoundError(
                    '`torchvision` is not installed yet.'
                )
        ...
        def default_transforms(self) -> Callable:
            if self.normalize:
                imagenette_transforms = transform_lib.Compose([
                    transform_lib.Resize((224, 224)),
                    transform_lib.ToTensor(),
                    imagenette_normalization()
                ])
            else:
                imagenette_transforms = transform_lib.Compose([
                    transform_lib.Resize((224, 224)),
