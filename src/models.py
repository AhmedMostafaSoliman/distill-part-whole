...
from utils import exists, default, SupConLoss, Siren, plot_islands_agreement, levels_recon_loss
...
class Agglomerator(pl.LightningModule):
    def __init__(self,
        ...
        *,
        ...
        self.classification_head_from_last_level = nn.Sequential(
            ...
            nn.Dropout(p=0.5),
            nn.Linear(self.num_patches * FLAGS.patch_dim, 10)
        )
    ...
    def training_step(self, train_batch, batch_idx):
        image = train_batch[0]
        label = train_batch[1]
        embeds = train_batch[2]
        indices = train_batch[3]
        paths = []
        # get image_paths from the indices, the indices are the indices of the image_paths in the directory of the split images
        rootdir = 'datasets/imagenette2/train'
        for cls_idx, sample_idx in zip(label, indices):
        ...
        if(self.FLAGS.supervise):
            with torch.no_grad():
                top_level, _, _ = self.forward(image)
                self.features.append(list(top_level.data.cpu().numpy()))
                self.labels.append(label.cpu().numpy())
        else:
            top_level, toplot, encoded_levels_out = self.forward(image)
            if(self.FLAGS.plot_islands):
                #get current training step
                if self.global_step % 50 == 0:
                    fig1 = plot_islands_agreement(toplot, image[0,:,:,:])
                    fig2 = plot_islands_agreement(embeds[0,:,:,:], image[0,:,:,:])
        ...
        if(self.FLAGS.supervise):
            output = self.classification_head_from_last_level(top_level)
            ...
            loss = levels_recon_loss(encoded_levels_out, embeds)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        if(self.FLAGS.supervise):
            ...
            if(self.FLAGS.plot_islands):
                ...
                if(step % 500 == 0):
                    #fig = plot_islands_agreement(toplot, image[0,:,:,:])
                    #wandb.log({"Islands": fig})
                    pass
